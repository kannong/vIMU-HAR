...
class Train_Test_Spilt_Rawdata():
    def __init__(self, filepath, train_size, object_name, window_size, window_step):
        ...
        self.label_num = 1
        self.raw_features_num = self.raw_window_data[0].values.shape[1] - 1 - self.label_num
    ...
    def data_process(self):
        ...
        for i in range(window_len):
            ...
            column_num -= 3  # 剔除角速度
            column_num -= 1  # 剔除末空列
            one_window = np.hstack((one_window[:, 0:3], one_window[:, 6:(one_window.shape[1] - 1)]))
...
# 单独运行Python时用
# path = 'E:/unity_pro/My_workV0.0/Assets/StreamingAssets/SourcesFolder/IMUSim/Walking_r_upleg_JNT_label_data.csv'
# spilt_test(filepath=path, train_spilt=4.0, test_spilt=1.0,
#            object_name='Walking_r_upleg_JNT', window_size=40, window_step=20)
# path = 'E:/unity_pro/My_workV0.0/Assets/StreamingAssets/SourcesFolder/IMUReal/object_data/Reverse_Lunge_Right Leg_r_upleg_JNT_imureal_data.csv'
# spilt_test(filepath=path, train_spilt=3.0, test_spilt=1.0,
#            object_name='Reverse_Lunge_Right Leg_r_upleg_JNT_imureal', window_size=40, window_step=20)
