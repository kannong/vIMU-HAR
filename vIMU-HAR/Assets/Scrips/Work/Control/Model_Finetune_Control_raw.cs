...
public class Model_Finetune_Sub_Control_raw : MonoBehaviour
{
    ...
    private List<UnityEngine.UI.Toggle> toggles_list = new List<UnityEngine.UI.Toggle>();
    ...
    public void Start_Sample()
    {
        ...
        csv_path = csvf.BinSourcesFolder + object_name + "_" + "imureal_data.csv";
        // 新增：是否列表中已经存在对象，存在的话则在原来的文件上面添加就可以
        ...
        labels_list.Add(labels_if.text);
        // print message
        message_t.text = "";
    ...
    public void Sample_Create()
    {
        ...
        for (int i = 0; i < toggles_list.Count; i++)
        {
            if (toggles_list[i].isOn)  // if toggle is on
            {
                ...
                para2[0] = csvf.BinSourcesFolder + objects_list[i] + "_" + "imureal_data_feas.csv";
                para2[1] = Data_preprocessing_Control.train_spilt;
                para2[2] = Data_preprocessing_Control.test_spilt;
                para2[3] = objects_list[i] + "_" + "imureal";
                Connect_Python_func.RunPythonScript(@"E:\unity_pro\My_workV0.0\Assets\Scrips\Work\py\train_test_spilt_func.py", para2);
                ...
                para[0] = csvf.BinSourcesFolder + objects_list[i] + "_" + "imureal_data.csv";
                para[1] = Data_preprocessing_Control.train_spilt;
                para[2] = Data_preprocessing_Control.test_spilt;
                ...
                Connect_Python_func.RunPythonScript(@"E:\unity_pro\My_workV0.0\Assets\Scrips\Work\py\train_test_spilt_rawdata.py", para);
    ...
    {
        // Use Python
        string[] para = new string[5+ Data_annotation_Control.label_num];
        para[0] = Model_Processing_Control.object_model; // object model
        para[1] = (Model_Processing_Control.object_model == "NN") ? NN_.NN.data_source : CNN_.CNN.data_source; // data source
        para[2] = (Model_Processing_Control.object_model == "NN") ? NN_.NN.loss : CNN_.CNN.loss;
        para[3] = Data_annotation_Control.label_num.ToString();
        for (int i = 0; i < Data_annotation_Control.label_num; i++)
        {
            para[4 + i] = Data_annotation_Control.labels[i];
        }
        Connect_Python_func.RunPythonScript(@"E:\unity_pro\My_workV0.0\Assets\Scrips\Work\py\model_fine_tune.py", para);
    }
    ...
    public void Data_Synthesise()
    {
        ...
        data_source_t.text = (Model_Processing_Control.object_model == "NN") ? NN_.NN.data_source : CNN_.CNN.data_source;
    ...
    public void Feature_Extract()
    {
        ...
        {
            // read csv, get corresponding windows datas
            string raw_data_path = csvf.BinSourcesFolder + objects_list[i] + "_" + "imureal_data.csv";
            string new_data_path = csvf.BinSourcesFolder + objects_list[i] + "_" + "imureal_data_feas.csv";
